...
class SqluserframeWidget(ttk.Frame):
    def __init__(self, master=None, **kw):
        ...
        self.sqlUserTree.configure(
            columns=self.sqlUserTree_cols, displaycolumns=self.sqlUserTree_dcols
        )
        self.sqlUserTree.column(
            "column1", anchor="w", stretch=True, width=100, minwidth=20
        )
        self.sqlUserTree.column(
            "column2", anchor="w", stretch=True, width=100, minwidth=20
        )
    ...
    def save_user(self):
        ...
        if not sels:  # 新建用户并赋予权限
            ...
            if userName != oldUserName or host != oldHost:
                sql = (
                    f"rename user '{oldUserName}'@'{oldHost}' to '{userName}'@'{host}';"
                )
                sqlM.execute_and_commit("mysql", sql)
            # get sql version
            sql = "select version()"
