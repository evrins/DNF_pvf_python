...
class StringTable:
    ...
    def __getitem__(self, n):
        ...
        if self.converted:
            ...
            value = convert(
                self.StringTableStrIndex[StrIndex[0] : StrIndex[1]].decode(
                    self.encode, 'ignore'
                ),
                'zh-cn',
            )
    ...
    def convertZhcn(self):
        ...
        for n in range(self.length * 2):
            StrIndex = struct.unpack('<II', self.StringTableStrIndex[n * 4 : n * 4 + 8])
            value = self.StringTableStrIndex[StrIndex[0] : StrIndex[1]].decode(
                self.encode, 'ignore'
            )
...
class Lst_lite2:
    ...
    def get_N_Str(self, n) -> Str:
        ...
        if res is None:
            ...
            if content is None:
                content = self.tinyPVF.read_File_In_Decrypted_Bin(
                    self.tableDict[n].lower()
                )
            self.strDict[n] = Str(
                content.decode(self.encode, 'ignore')
            )  # self.getStr(path=self.tableDict[n])
            res = self.strDict.get(n)
        return res
    ...
    def content2List(
        ...
        for i in range(unit_num):
            ...
            if types[i] in [2]:
                ...
                valuesRead.append(
                    nString.get_N_Str(values[i])[stringtable[values[i + 1]]]
                )
            else:
                typesInList.pop(-1)
        if convertZhcn:
        ...
        for i, value in enumerate(fileInList):
            if typeList[i] == 5:
                ...
                if endMarkFlg and value.replace('/', '') != segmentKey.split('-')[0]:
                    ...
                    if (
                        len(segment) > 0 or '[/' in value[:3] or segmentKey is None
                    ):  # or segmentKey=='[possible kiri protect]'
                        segmengFin = True
                    else:
                        segmengFin = False
                ...
                if segmengFin:  # 是新的段，保存数据，判断新段类型
                    ...
                    if segmentKey is not None:
                        if segmentInSegmentFlg and len(segment) > 1:
                            res[segmentKey] = TinyPVF.list2Dict(
                                [segTypes, segment], segmentKey
                            )
                        else:
                        ...
                        if GEN_KEYWORD:
                            if (
                                subKeywordsDict.get(segmentKey) != endMarkFlg
                            ):  # 优先保存为True，表示有段落结束符
                                if (
                                    subKeywordsDict.get(segmentKey) == False
                                    or subKeywordsDict.get(segmentKey) is None
                                ):
                                    subKeywordsDict[segmentKey] = endMarkFlg
                            if parentKey is not None:  # 保存母子节点信息
                                if (
                                    subKeywordsDict['__segInSegKeys__'].get(parentKey)
                                    is None
                                ):
                                    ...
                                    if (
                                        ...
                                        for j in range(i + 1, len(segment)):
                                            ...
                                            if (
                                                ...
                                                tmp.append(value__)
                                        if (
                                            subKeywordsDict['__quoteInSegKeys__'].get(
                                                segmentKey
                                            )
                                            is None
                                        ):
                                        ...
                                        oldValue = subKeywordsDict[
                                            '__quoteInSegKeys__'
                                        ][segmentKey].get(value_)
                                        if oldValue is None:
                                            subKeywordsDict['__quoteInSegKeys__'][
                                                segmentKey
                                            ][value_] = []
                                        if (
                                            len(tmp) == 1
                                            and tuple(tmp)
                                            not in subKeywordsDict[
                                                '__quoteInSegKeys__'
                                            ][segmentKey][value_]
                                        ):
                                            subKeywordsDict['__quoteInSegKeys__'][
                                                segmentKey
                                            ][value_].append(tuple(tmp))
                                        elif (
                                            len(tmp) > 0
                                            and tuple(tmp)
                                            not in subKeywordsDict[
        ...
        for i, value in enumerate(fileInList):
            if typeList[i] == 5:
                ...
                if segmentKey is None:
                    ...
                    if (
                        segmentKey not in segmentKeysWithEndMark
                        or value.replace('/', '') == segmentKey
                    ):
                        add_seg()
                        segmentKey = value if '/' not in value else None
                        segTypes = []
        ...
        while i < len(typeList):
            if typeList[i] == 5:
                if i in keyIndexListWithEndMark:  # 寻找结束符，然后递归调用
                    ...
                    endIndex = keyIndexListWithEndMark_END[
                        keyIndexListWithEndMark.index(i)
                    ]
                    subValuesList = valuesList[i + 1 : endIndex]
                    subTypesList = typeList[i + 1 : endIndex]
                    segment.append(
                        {
                            subSegmentKey: TinyPVF.list2StructedList(
                                [subTypesList, subValuesList]
                            )
                            + [True]
                        }
                    )
    ...
    def dictSegment2text(
        ...
        for key, segment in keyAndSegList:
            ...
            if isinstance(segment, dict):
                res += TinyPVF.dictSegment2text(
                    segment, prefix + prefixAdd, depth=depth - 1
                )  # + prefix + '/'+ key + '\n'
            else:
                tmpres = ''
        ...
        for i in range(len(res[1]) - 1):
            ...
            if postFixStart:
                if isinstance(res[1][i], int) and '/' not in str(res[1][i + 1]):
                    try:
                        magicSealDict[res[1][i]] = zhconv.convert(
                            res[1][i + 1]
                            .replace('[', '')
                            .replace(']', '')
                            .split(':')[0],
...
def get_exp_table2(pvf: TinyPVF):
    try:
        ...
        expTableList = list(
            filter(lambda value: isinstance(value, int), expTableList[1])
        )
...
def get_Equipment_Dict_multi(args):  # 分为x个part
    ...
    fileListInTask = ItemLst.tableList[
        taskPart * length // allPartNum : (taskPart + 1) * length // allPartNum
    ]
