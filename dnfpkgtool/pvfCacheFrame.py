...
class PVFCacheCfgFrame(TitleBarFrame):
    def __init__(
        ...
        if True:
            ...
            ttk.Button(editFrame, text='导出', command=self.exportEqu).grid(
                row=row, column=2, sticky='e', padx=kw['padx']
            )
            row += 1
            ...
            ttk.Button(editFrame, text='导出', command=self.exportStk).grid(
                row=row, column=2, sticky='e', padx=kw['padx']
            )
            row += 1
            btnFrame = tk.Frame(editFrame)
            btnFrame.grid(row=row, column=1, columnspan=2)
            ttk.Button(btnFrame, text='加载', command=self.fillJsonTree).grid(
                row=row, column=0, **kw
            )
            ttk.Button(btnFrame, text='保存', command=self.renameCache).grid(
                row=row, column=1, **kw
            )
        ...
        jsonTree = ttk.Treeview(
            jsonTreeFrame, selectmode='browse', height=10, columns='Values'
        )
    ...
    def fillJsonTree(self):
        def inner():
            ...
            self.jsonTreeFrame.config(
                text='缓存浏览器-' + pvfCacheDict.get('nickName') + '-' + MD5
            )
            showDict = {
    ...
    def exportEqu(self):
        ...
        filePath = asksaveasfilename(
            ...
            initialfile=f'{self.nickNameE.get()}-装备.{fileType}',
        )
        if filePath == '':
            return False
    ...
    def exportStk(self):
        ...
        filePath = asksaveasfilename(
            title=f'保存文件(.{fileType})',
            filetypes=[('二进制文件', f'*.{fileType}')],
            initialfile=f'{self.nickNameE.get()}-道具.{fileType}',
        )
        if filePath == '':
            return False
        if filePath[-1 - len(fileType) :] != f'.{fileType}':
