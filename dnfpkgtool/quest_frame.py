...
class QuestFrameWidget(ttk.Frame):
    ...
    def search_quest(self, e: tk.Event):
        ...
        if len(key) > 0:
            questList = [
                [questID, questDict.get("[name]", [questID])[0]]
                for questID, questDict in cacheM.PVFcacheDict.get("quest", {}).items()
            ]
            # print(questList[:10])
            res = cacheM.searchItem(key, questList)
            self.questNameE.config(
    ...
    def show_sel_quest(self, event=None):
        ...
        questIndex, questID, questName, questTrigger = self.questTree.item(
            sel, "values"
        )
