...
class DnfItemSlot:
    ...
    def __init__(self, item_bytes: bytes = b'') -> None:
        ...
        self.coverMagic = self.magicSeal[
            -1
    ...
    def __repr__(self) -> str:
        ...
        if self.typeDict.get(self.type) in [
            '消耗品',
            '材料',
            '任务材料',
            '副职业',
...
def getAvatar(cNo, ability_=False):
    getAvatarSql = (
        f'select ui_id,it_id,hidden_option from user_items where charac_no={cNo};'
    )
...
def getCreatureItem(cName='', cNo=0):
    ...
    if cNo != 0:
        ...
        charac_no = getCharactorNo(cName)[0][0]
    get_creatures_sql = (
        f'select ui_id,it_id,name from creature_items where charac_no={charac_no};'
    )
...
def setInventory(InventoryBlob, cNo, key='inventory'):
    ...
    if key in ['inventory', 'equipslot', 'creature']:
        table = 'inventory'
    if key in ['cargo', 'jewel', 'expand_equipslot']:
...
def set_baned(uid, days=365, punish_type=1, punishValue=101, reason=''):
    ...
    end_time = (datetime.datetime.now() + datetime.timedelta(days=days)).strftime(
        '%Y-%m-%d %H:%M:%S'
    )
        ...
        try:
            ...
            if key == 'charac_name':
                ...
                try:
                    ...
                    )  # .decode(SQL_ENCODE_LIST[sqlEncodeUseIndex])
                except:
                    value = convert(value, 'zh-tw').encode(
                        ENCODE, errors='replace'
                    )  # .decode(SQL_ENCODE_LIST[sqlEncodeUseIndex])
                charSet = 'latin1'
...
def send_postal(
    ...
    def send():
        ...
        if avata_flag == 1:
            ...
            num = ui_id
        sql = (
            'insert into postal (occ_time,send_charac_name,receive_charac_no,amplify_option,amplify_value,seperate_upgrade,seal_flag,item_id,add_info,upgrade,gold,letter_id,avata_flag,creature_flag,endurance,unlimit_flag) '
...
def get_postal_new(cNo):
    ...
    for (
        ...
        *_,
        ...
        res.append(
            [postal_id, 'itemName', item_id, decode(send_charac_name), num, type]
        )
    return res
        ...
        for i, connector_ in enumerate(SQL_CONNECTOR_LIST):
            try:
                db = connector_.connect(
                    ...
                    charset='utf8',
                    autocommit=True,
                    **SQL_CONNECTOR_IIMEOUT_KW_LIST[i],
                )
