...
class App:
    def __init__(self, root: tk.Tk, check_update=True):
        ...
        if HEIGHT != 1:
            ...
            w.defaultFont.configure(
                family="黑体",
                size=int(3 * (HEIGHT - 1.3) + 11.5),
            )  #
            ttk.Style().configure(
                "TEntry",
                padding=6,
    ...
    def _buildtab_main(self, tabView):
        ...
        def searchCharac(searchType="account"):  # 或者cName
            if searchType == "account":
                characs = sqlM.getCharacterInfo(uid=sqlM.getUID(self.accountE.get()))
                try:
                ...
                if self.characE.get() == "":
                    ...
                    characs = sqlM.getCharacterInfo(cName=self.characE.get())
            ...
            def inner():
                ...
                if cacheM.config.get("PVF_PATH") == "":
                    messagebox.askokcancel(
                        "这是一个初次运行的广告",
                        "全服背包管理工具赞助即可获得！详情请点击GM工具->赞助投喂页面。",
                    )
                ...
                if pvfPath != "":
                    ...
                    info = cacheM.loadItems2(
                        True, pvfPath, encode=self.pvfEncodeBox.get()
                    )
        ...
        if True:  # 左侧查询
            ...
            accountBtn = ttk.Button(
                accountSearchFrame,
                text="查询/加载所有",
                command=lambda: searchCharac("account"),
                state="disable",
            )
            ...
            characBtn = ttk.Button(
                characSearchFrame,
                text="查询/加载在线",
                command=lambda: searchCharac("cName"),
                state="disable",
            )
            ...
            connectorE = ttk.Combobox(
                connectorFrame, width=int(WIDTH * 10), state="readonly"
            )
            ...
            self.connectorE.set("----")
            encodeE = ttk.Combobox(
                connectorFrame,
                width=int(WIDTH * 10),
                values=[
                    f"{i}-{encode}" for i, encode in enumerate(sqlM.SQL_ENCODE_LIST)
                ],
                state="readonly",
            )
            ...
            ttk.Button(
                PVFSelFrame, text="读取PVF文件", command=lambda: loadPVF("")
            ).pack(anchor="w", padx=PADX * 5, pady=PADY * 3, fill="x", expand=True)
            res = []
            for MD5, infoDict in list(cacheM.cacheManager.tinyCache.items()):
                if not isinstance(infoDict, dict):
                    continue
                res.append(f"{cacheM.cacheManager.tinyCache[MD5]['nickName']}-{MD5}")
            pvfComboBox = ttk.Combobox(
                PVFSelFrame, values=res, width=int(WIDTH * 10), state="readonly"
            )
            pvfComboBox.set("请选择PVF缓存")
            pvfComboBox.pack(
                anchor="w", padx=PADX * 5, pady=PADY * 3, fill=fill, expand=True
            )
            pvfComboBox.bind(
                "<<ComboboxSelected>>",
                lambda e: loadPVF(pvfComboBox.get().split("-")[-1]),
            )
            self.pvfComboBox = pvfComboBox
            CreateToolTip(pvfComboBox, "PVF缓存")

            pvfEncodeBox = ttk.Combobox(
                PVFSelFrame,
                values=["big5", "gbk", "utf-8"],
                width=int(WIDTH * 10),
                state="readonly",
            )
        ...
        padFrame.grid(
            row=1, column=2, rowspan=5, sticky="ns", padx=PADX * 5, pady=PADY * 5
        )

        middleFrame = tk.Frame(searchFrame)
        middleFrame.grid(
            row=1, column=2, rowspan=5, sticky="nswe", padx=PADX * 5, pady=PADY * 5
        )
        characTreev = ttk.Treeview(
            middleFrame, selectmode="browse", height=int(HEIGHT * 17)
        )
        ...
        infoFrame_ = tk.Frame(
            searchFrame, width=int(WIDTH * 192), height=int(HEIGHT * 5)
        )
        infoFrame_.grid(row=0, column=3, sticky="nwse")
        if len(cacheM.config["TITLE"]) > 0:
            tk.Label(
                infoFrame, text=cacheM.config["TITLE"], font=cacheM.config["FONT"][0]
            ).pack(anchor="n", side="top")
        if len(cacheM.config["VERSION"]) > 0:
            tk.Label(
                infoFrame, text=cacheM.config["VERSION"], font=cacheM.config["FONT"][1]
            ).pack(anchor="n", side="top")
        if len(cacheM.config["INFO"]) > 0:
            tk.Label(
                infoFrame, text=cacheM.config["INFO"], font=cacheM.config["FONT"][2]
            ).pack(anchor="n", side="top")
        ...
        for tabName in self.globalCharacBlobs.keys():
            ...
            for index, itemSlot in itemDict.items():
                ...
                if tabName == "物品栏" and typeID != 0:
                    if index in range(*positionDict[typeID][1]) or index in [
                        3,
                        4,
                        5,
                        6,
                        ...
                        self.errorItemsListDict[tabName].append(index)
                        self.errorInfoDict[tabName][index] += (
                            f"物品位置错误-当前{index}-{positionDict[typeID][1]} \n"
                        )
                elif tabName == "穿戴栏":
                    if typeID != 0x01:
                        self.errorItemsListDict[tabName].append(index)
    ...
    def _buildtab_itemTab(self, tabView, tabName, treeViewArgs):
        ...
        def config_TreeViev(
            ...
            itemsTreev.bind(
                "<<TreeviewSelect>>", lambda e: self.w.after(100, singleFunc)
            )
        ...
        def showSelectedItemInfo(save=True, reset=False):
            ...
            if reset:
                try:
                    index = int(
                        itemEditFrame.cget("text").split("(")[-1].replace(")", "")
                    )
                    print(index)
                except:
                    return False
            ...
            if save and self.editedItemsDict.get(tabName).get(index) is not None:
                ...
                itemSlot: sqlM.DnfItemSlot = self.selectedCharacItemsDict[tabName][
                    index
                ]
        ...
        def editSave(retType="bool"):
            ...
            for i in range(4):
                magicSeals.append(
                    [
                        int(magicSealIDEntrys[i].get()),
                        magicSealEntrys[i].get(),
                        int(magicSealLevelEntrys[i].get()),
                    ]
                )
        ...
        invBowserFrame = tk.LabelFrame(
            inventoryFrame, text="当前物品列表", width=int(WIDTH * 306)
        )
        ...
        if True:  #'当前物品列表'
            ...
            if True:
                ...
                ttk.Checkbutton(
                    filterFrame,
                    text="显示空槽位",
                    variable=emptySlotVar,
                    command=refill_Tree_View,
                ).pack(side="left", padx=PADX * 10)
            ...
            if True:
                ...
                padFrame = tk.Frame(
                    treeViewFrame, height=int(HEIGHT * 326 + (HEIGHT - 1) * 80), width=4
                )  # 控制treeview高度int(WIDTH*4),bg='red'
                padFrame.pack(side=tk.LEFT, fill="y")

                itemsTreev_now = ttk.Treeview(
                    treeViewFrame, selectmode="browse", height=int(HEIGHT * 11)
                )
                ...
                if True:
                    ...
                    itemsTreev_now.config(
                        yscrollcommand=sbar1.set, xscrollcommand=sbar1.set
                    )
            ...
            if True:
                ...
                exportBtn.pack(
                    side="left", fill="both", expand=True, padx=PADX * 5, pady=PADY * 3
                )
                ...
                importBtn.pack(
                    side="right", fill="both", expand=True, padx=PADX * 5, pady=PADY * 3
                )
                CreateToolTip(importBtn, text="从文件导入数据并覆盖")
        # return False
        ttk.Separator(inventoryFrame, orient="vertical").grid(
            row=1, column=2, rowspan=2, sticky="nswe"
        )

        itemEditFrame = tk.LabelFrame(
            inventoryFrame, text="物品信息编辑", width=int(WIDTH * 296)
        )
        ...
        itemNameEntry = ttk.Combobox(
            itemEditFrame, width=int(WIDTH * 14), state="normal"
        )
        itemNameEntry.bind("<Button-1>", searchItem)
        itemNameEntry.grid(
            column=2, row=row, sticky="nswe", padx=PADX * padx, pady=PADY * pady
        )
        ...
        itemIDEntry.grid(
            column=3, row=row, sticky="nswe", padx=PADX * padx, pady=PADY * pady
        )
        ...
        numEntry = ttk.Spinbox(
            itemEditFrame, width=int(WIDTH * 15), from_=0, to=4294967295
        )
        numEntry.grid(
            column=2, row=row, sticky="nswe", padx=PADX * padx, pady=PADY * pady
        )
        ...
        tk.Label(itemEditFrame, text=" 耐久：").grid(
            column=3, row=row, sticky="nsw", padx=PADX * padx, pady=PADY * pady
        )
        durabilityEntry = ttk.Spinbox(
            itemEditFrame, width=int(WIDTH * 4), from_=0, to=999
        )
        durabilityEntry.grid(
            column=3, row=row, sticky="nse", padx=PADX * padx, pady=PADY * pady
        )
        # 4
        row = 4
        tk.Label(itemEditFrame, text="增幅：").grid(
            column=1, row=row, padx=PADX * padx, pady=PADY * pady
        )
        IncreaseTypeEntry = ttk.Combobox(
            itemEditFrame,
            width=int(WIDTH * 14),
            state="readonly",
            values=[
        ...
        IncreaseTypeEntry.grid(
            column=2, row=row, sticky="nswe", padx=PADX * padx, pady=PADY * pady
        )
        IncreaseEntry = ttk.Spinbox(
            itemEditFrame, width=int(WIDTH * 10), from_=0, to=65535
        )
        IncreaseEntry.grid(
            column=3, row=row, sticky="nswe", padx=PADX * padx, pady=PADY * pady
        )
        ...
        # 5
        row = 5
        tk.Label(itemEditFrame, text="强化：").grid(
            column=1, row=row, padx=PADX * padx, pady=PADY * pady
        )
        EnhanceEntry = ttk.Spinbox(
            itemEditFrame, width=int(WIDTH * 15), increment=1, from_=0, to=31
        )
        EnhanceEntry.grid(
            column=2, row=row, sticky="nswe", padx=PADX * padx, pady=PADY * pady
        )
        ...
        delBtn.grid(
            row=row, column=3, pady=PADY * pady, padx=PADX * padx, sticky="nswe"
        )
        # 6
        row = 6
        tk.Label(itemEditFrame, text="锻造：").grid(
            column=1, row=row, padx=PADX * padx, pady=PADY * pady
        )
        forgingEntry = ttk.Spinbox(
            itemEditFrame, width=int(WIDTH * 15), increment=1, from_=0, to=31
        )
        forgingEntry.grid(
            column=2, row=row, sticky="nswe", padx=PADX * padx, pady=PADY * pady
        ...
        enableTestBtn = ttk.Checkbutton(
            itemEditFrame,
            text=" 启用种类字段",
            variable=enableTypeChangeVar,
            command=enableTestFrame,
        )
        enableTestBtn.grid(
            column=1,
            row=row,
        ...
        tk.Label(itemEditFrame, text="种类:").grid(
            column=1,
            row=row,
            columnspan=2,
            sticky="e",
        ...
        if True:
            ...
            padx = 1
            row = 1
            tk.Label(equipmentExFrame, text="异界").grid(
                column=1, row=row, padx=PADX * padx, pady=PADY * pady
            )
            otherworldEntry = ttk.Entry(
                equipmentExFrame, state="readonly", width=int(WIDTH * 29)
            )
            otherworldEntry.grid(
                column=2,
                row=row,
                columnspan=5,
                sticky="nswe",
            ...
            row = 2
            tk.Label(equipmentExFrame, text="宝珠").grid(
                column=1, row=row, padx=PADX * padx, pady=PADY * pady
            )
            orbTypeEntry = ttk.Combobox(
                equipmentExFrame, state="readonly", width=int(WIDTH * 10)
            )
            orbValueEntry = ttk.Combobox(
                equipmentExFrame, state="readonly", width=int(WIDTH * 11)
            )
            orbEntry = ttk.Entry(
                equipmentExFrame, state="readonly", width=int(WIDTH * 7)
            )  #
            orbTypeEntry.grid(
                column=2, row=row, padx=PADX * padx, pady=PADY * pady, sticky="nswe"
            )
            orbValueEntry.grid(
                column=3,
                row=row,
            ...
            # 8-3
            row = 3
            tk.Label(equipmentExFrame, text="魔法封印：").grid(
                column=1,
                row=row,
                columnspan=2,
                sticky="w",
            ...
            forthSealEnable.set(1)
            forth = ttk.Checkbutton(
                equipmentExFrame,
                variable=forthSealEnable,
                text="第四词条",
                command=lambda: print(forthSealEnable.get()),
            )
            ...
            def build_magic_view(row=4):
                magicSealEntry = ttk.Combobox(
                    equipmentExFrame,
                    state="normal",
                    width=int(WIDTH * magicSealEntryWidth),
                    values=list(cacheM.magicSealDict.values()),
                )
                magicSealIDEntry = ttk.Entry(
                    equipmentExFrame, state="readonly", width=int(WIDTH * 4)
                )
                magicSealLevelEntry = ttk.Spinbox(
                    equipmentExFrame,
                    state="normal",
                    width=int(WIDTH * 7),
                    from_=0,
                ...
                magicSealEntry.bind(
                    "<Button-1>", lambda e: searchMagicSeal(magicSealEntry)
                )
                magicSealEntry.bind(
                    "<<ComboboxSelected>>",
                ...
                magicSealIDEntry.grid(
                    column=4, row=row, sticky="nswe", padx=PADX * padx, pady=PADY * pady
                )
                magicSealLevelEntry.grid(
                    column=5,
                    row=row,
                    sticky="nswe",
                    padx=PADX * padx,
        ...
        if True:
            ...
            itemSlotBytesE.pack(
                side="left", fill="both", expand=True, padx=PADX * 2, pady=PADY * 3
            )  # grid(row=2,column=2,padx=PADX*2)
            CreateToolTip(
            ...
            genBytesBtn = ttk.Button(
                btnFrame, text="生成字节", command=genBytes, width=int(WIDTH * 7)
            )
            genBytesBtn.pack(
                side="left", fill="both", expand=True, padx=PADX * 2, pady=PADY * 3
            )  # .grid(row=2,column=1,padx=PADX*2)
            ...
            importBtn = ttk.Button(
                btnFrame, text="导入字节", command=readBytes, width=int(WIDTH * 7)
            )
            importBtn.pack(
                side="left", fill="both", expand=True, padx=PADX * 2, pady=PADY * 3
            )  # .grid(row=2,column=3,padx=PADX*2)
            CreateToolTip(importBtn, "读取字节，导入到编辑框\n用于物品复制")
            commitBtn = ttk.Button(
                btnFrame, text="提交修改", command=ask_commit, width=int(WIDTH * 7)
            )
            commitBtn.pack(
                side="left", fill="both", expand=True, padx=PADX * 2, pady=PADY * 3
            )  # .grid(row=2,column=4,pady=PADY*5)
    ...
    def _buildtab_itemTab_2(self, tabView, tabName, treeViewArgs):
        ...
        def set_TreeView_Func(
            ...
            def release_Tree_now(e):
                ...
                if abs(e.x - x) + abs(e.y - y) < 10:
                    ...
                    itemsTree_now.winfo_width() * 2
                    > e.x
                    > itemsTree_now.winfo_width() + 5
                    and 0 < e.y < itemsTree_now.winfo_height()
                ):
            ...
            def release_Tree_edit(e):
                ...
                if abs(e.x - x) + abs(e.y - y) < 10:
                    ...
                    0 - itemsTree_edit.winfo_width() < e.x < 0
                    and 0 < e.y < itemsTree_now.winfo_height()
                ):
            ...
            itemsTree_now.bind(
                "<Double-1>", lambda e: addDel(itemsTree_now, itemsTree_edit)
            )
        ...
        allInventoryFrame = tk.LabelFrame(
            inventoryFrame, text="当前物品列表", width=int(WIDTH * 306)
        )
        ...
        padFrame = tk.Frame(
            allInventoryFrame, height=int(HEIGHT * 380), width=int(WIDTH * 4)
        )
        ...
        delInventoryFrame = tk.LabelFrame(
            inventoryFrame, text="待修改物品列表", width=int(WIDTH * 295)
        )
        ...
        itemsTreev_del = ttk.Treeview(
            delInventoryFrame, height=int(HEIGHT * 15 + (HEIGHT - 1) * 5)
        )  # if tabName==' 宠物 ' else 13
        itemsTreev_del.grid(
            row=1,
            column=2,
            rowspan=2,
        ...
        resetBtn = ttk.Button(
            delInventoryFrame,
            text=" 重选 ",
            command=lambda: reselect(tabName),
            width=int(WIDTH * 10) if tabName == " 宠物 " else 8,
        )
        resetBtn.grid(row=4, column=2, pady=PADY * pady)
        if "时装" in tabName:
            hiddenCom = ttk.Combobox(
                delInventoryFrame,
                values=["0-None"]
                + [
                    f"{i + 1}-{value}"
            ...
            addHiddenBtn = ttk.Button(
                delInventoryFrame,
                text="开启潜能",
                command=lambda: enableHidden(tabName, itemsTreev_del),
                width=int(WIDTH * 8),
            )
        ...
        delBtn = ttk.Button(
            delInventoryFrame,
            text="确定删除",
            command=lambda: deleteItems(tabName, itemsTreev_del),
            width=int(WIDTH * 10) if tabName == " 宠物 " else 8,
        )
    ...
    def _buildtab_charac(self, tabView, tabName):
        ...
        if True:
            fill = "x" if HEIGHT == 1 else "both"
            otherFunctionFrame = tk.LabelFrame(
                characEntriesAndGitHubFrame, text="附加功能"
            )
            otherFunctionFrame.pack(
                padx=PADX * 5, pady=PADY * 5, side="left", fill="both", expand=True
            )
            btn = ttk.Button(
                otherFunctionFrame, text="生成一键启动器", command=ps.saveStart
            )
            ...
            btn = ttk.Button(
                otherFunctionFrame,
                text="PVF缓存管理器",
                command=self.open_PVF_Cache_Edit,
            )
            btn.pack(expand=True, fill=fill, padx=PADX * 5)
            CreateToolTip(btn, "修改缓存数据\n导出装备道具列表为CSV")

            btn = ttk.Button(
                otherFunctionFrame, text="PVF工具", command=self._open_PVF_Editor
            )
            ...
            sizeE_W = ttk.Combobox(
                sizeFrame, values=[1, 1.3, 1.5, 1.8, 2, 2.5], width=int(WIDTH * 3)
            )
            sizeE_W.set(WIDTH)
            ...
            updateCheckVar.set(
                1
                if cacheM.config.get("UPDATE_SUTO") is None
                or cacheM.config.get("UPDATE_SUTO") == 1
                else 0
            )
            ...
            if True:
                ...
                tk.Label(characEntriesFrame, text="角色名：").grid(
                    row=row, column=3, padx=PADX * padx, pady=PADY * pady
                )
                nameE = ttk.Entry(
                    characEntriesFrame, width=int(WIDTH * entryWidth)
                )  # ,state='readonly'
                nameE.grid(
                    row=row, column=4, padx=PADX * padx, pady=PADY * pady, sticky="nswe"
                )
                self.nameEditE = nameE
                # CreateToolTip(self.nameEditE,'编码出现错误时无法修改角色名')

                row += 1
                tk.Label(characEntriesFrame, text="角色等级：").grid(
                    row=row, column=3, padx=PADX * padx, pady=PADY * pady
                )
                levE = ttk.Spinbox(
                    characEntriesFrame,
                    from_=1,
                    to=999,
                    width=int(WIDTH * entryWidth / 1.7),
                )
                levE.grid(
                    row=row, column=4, padx=PADX * padx, pady=PADY * pady, sticky="nsw"
                )
                isVIP = tk.IntVar()
                ttk.Checkbutton(
                    characEntriesFrame,
                    text="VIP账户",
                    variable=isVIP,
                    command=lambda: isVIP.get(),
                ).grid(
                    row=row, column=4, padx=PADX * padx, pady=PADY * pady, sticky="nse"
                )
                row += 1
                ...
                tk.Label(characEntriesFrame, text="职业：").grid(
                    row=row, column=3, padx=PADX * padx, pady=PADY * pady
                )
                jobE2 = ttk.Combobox(
                    characEntriesFrame,
                    width=int(WIDTH * entryWidth / 3),
                    values=[f"{key}-{value}" for key, value in expert_jobMap.items()],
                )
                jobE2.grid(
                    row=row, column=4, padx=PADX * padx, pady=PADY * pady, sticky="nse"
                )
                jobE = ttk.Combobox(
                    characEntriesFrame,
                    width=int(WIDTH * entryWidth / 1.75),
                    values=[
                        f"{item[0]}-{item[1][0]}" for item in cacheM.jobDict.items()
                    ],
                )
                ...
                self.jobE = jobE
                row += 1
                tk.Label(characEntriesFrame, text="成长类型：").grid(
                    row=row, column=3, padx=PADX * padx, pady=PADY * pady
                )
                growTypeE = ttk.Combobox(
                    characEntriesFrame, width=int(WIDTH * entryWidth)
                )
                growTypeE.grid(
                    row=row, column=4, padx=PADX * padx, pady=PADY * pady, sticky="nswe"
                ...
                row += 1
                tk.Label(characEntriesFrame, text="觉醒标识：").grid(
                    row=row, column=3, padx=PADX * padx, pady=PADY * pady
                )
                wakeFlgE = ttk.Combobox(
                    characEntriesFrame,
                    state="readonly",
                    values=[0, 1],
                    width=int(WIDTH * entryWidth / 1.75),
                ...
                ttk.Checkbutton(
                    characEntriesFrame,
                    text="回归玩家",
                    variable=isReturnUser,
                    command=lambda: isReturnUser.get(),
                ).grid(
                    row=row, column=4, padx=PADX * padx, pady=PADY * pady, sticky="nse"
                )
                row += 1
                ttk.Button(characEntriesFrame, text="提交修改", command=commit).grid(
                    row=row, column=3, columnspan=2, sticky="nswe"
                )
            GitHubFrame(characEntriesAndGitHubFrame).pack(
                padx=PADX * 5, pady=PADY * 5, side="right"
            )
    ...
    def _open_GM(self, sshAutoFlg=True):
        ...
        self.GMTool = gmToolGUI.GMToolWindow(
            self.tabView,
            cNo=self.cNo,
            sponsorFrame=self.sponsorFrame,
            sshAutoConnect=sshAutoFlg,
        )
    ...
    def open_advance_search_equipment(self):
        def start_Search():
                ...
                if type1 in ["首饰", "特殊装备"]:
                    ...
                    if type2 == "":
                        ...
                        if type3 == "":
                            ...
                            for id in cacheM.equipmentForamted[type1][type2][
                                type3
                            ].keys():
                                typeDict[id] = type3
            res = []
            ...
            if nameKey != "":
                ...
                searchList = cacheM.searchItem(
                    nameKey, list(searchDict.items()), fuzzy=useFuzzy
                )
            else:
        ...
        advanceSearchMainWin.wm_geometry(
            "+%d+%d"
            % (self.advanceSearchBtn.winfo_rootx(), self.advanceSearchBtn.winfo_rooty())
        )
    ...
    def build_GUI(self, w):
        ...
        advanceSearchBtn = tk.Button(
            tabFrame,
            text="装备搜索",
            relief=tk.FLAT,
            font=("", 10, "underline"),
            command=self.open_advance_search_equipment,
        )
        self.advanceSearchBtn = advanceSearchBtn
        advanceSearchBtn.grid(row=1, column=1, sticky="ne", padx=PADX * 3)

        advanceSearchBtn2 = tk.Button(
            tabFrame,
            text="道具搜索",
            relief=tk.FLAT,
            font=("", 10, "underline"),
            command=self.open_advance_search_stackable,
        )
        ...
        self.refreshBtn = tk.Button(
            tabFrame, text="刷新背包", relief=tk.FLAT, font=("", 10, "underline")
        )
    ...
    def enable_Tabs(self):
        """启用上方tab"""
        for i in range(
            1, len(globalBlobs_map.keys()) + 2 + len(globalNonBlobs_map.keys())
        ):
        ...
        def inner():
            ...
            if updateState:
                ...
                if updateACK:
                    ...
                    self.titleLog(
                        f"正在下载最新版本...{updateM.versionDict_remote['VERSION']}"
                    )
