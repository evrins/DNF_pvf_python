...
class StringTableEditor(StringTable):
    ...
    def __init__(self, tableBytes: bytes, encode="big5") -> None:
        ...
        self.StringTableStrIndexBytes = bytearray(
            tableBytes[4 : 4 + self.length * 4 + 4]
        )  # 4+self.length*4*2
    ...
    def add(self, itemID=0, string=""):
        ...
        if itemID == 0 or itemID is None:
            while True:
                ...
                if (
                    self.tableDict.get(itemID) is None
...
class TinyPVFEditor(TinyPVF):
                ...
                if segmengFin:  # 是新的段，保存数据，判断新段类型
                    ...
                    if segmentKey is not None:
                        if (
                            ...
                            res[segmentKey] = [
                                TinyPVFEditor.list2Dict_with_bin(
                                    [segTypes, segment, binarys]
                                ),
                                binarys,
                            ]
                        else:
    ...
    def gen_File_chunk(self, uuid=b"", fileVersion=None):
        ...
        for leafType, lstInfo in LSTPATHDICT.items():
            ...
            lstObjDict[leafType] = LstEditor(
                self.read_File_In_Decrypted_Bin(lstPath),
                self.stringTable,
                baseDir=leafType,
                suffix=suffix,
            )
        ...
        fullFile = (
            buildHeader() + treeChunk_encrypt + fileChunk
        )  # + b'Hey, Vergil! Your portal opening days are over.'
        return fullFile
...
def test2():
    ...
    print(
        hex(
            zlib.crc32(
                pvfHeader.unpackedHeaderTreeDecrypted, pvfHeader.numFilesInDirTree
            )
        )
    )
    print(pvfHeader)
