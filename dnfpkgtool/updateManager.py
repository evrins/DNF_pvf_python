...
UPDATE_INFO_URL_LIST_2 = [
    r"https://raw.githubusercontent.com/Zageku/DNF_pvf_python/main/versionDict2.json",
]

versionDict = {"VERSION": "230623", "URL": "", "history": {"230325": "url"}}
# 发包测试
...
def gen_Update_Json(new_version, new_url, info=""):
    if (
        new_version == versionDict["VERSION"]
        or new_version in versionDict["history"].keys()
    ):  # 版本已存在
...
def check_Update(updatePaths=UPDATE_INFO_URL_LIST_2):
    ...
    for UPDATE_url in updatePaths:
        try:
            ...
            if (
                local_version in versionDict_remote["history"].keys()
                and local_version != versionDict_remote["VERSION"]
            ):  # 在历史版本且不是最新版
                ...
                versionDict_remote["VERSION"] not in versionDict["history"].keys()
                and local_version != versionDict_remote["VERSION"]
            ):  # 服务器版本不在本地列表中
                url: str = versionDict_remote["URL"]
                newFileName = "背包编辑工具-" + url.rsplit("/")[-1]
...
if __name__ == "__main__":
    ...
    info = """修复邮件管理页bug"""
    versionDict = gen_Update_Json(
        "230622",
        "http://127.0.0.1:8888/dnf_package_tool_0622_patch.zip",
        info + "\n[增量更新]下载后请解压覆盖文件",
    )
    print("新本地：", versionDict)
    versionDict = {"VERSION": "230313", "URL": "", "history": {"230313": "url"}}
    update_state = check_Update()
