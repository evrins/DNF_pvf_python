...
class GMToolWindow(tk.Toplevel):
    ...
    def buildTab_usual(self, tabView: ttk.Notebook, tabName: str):
        def charge(type="cera"):
            ...
            if type == "点券":
                ...
                sqlM.set_skill_sp(
                    self.cNo,
                    value + cNoInfoDict["sp"],
                    value + cNoInfoDict["sp2"],
                    cNoInfoDict["tp"],
                    cNoInfoDict["tp2"],
                )
            elif type == "TP":
                sqlM.set_skill_sp(
                    ...
                    cNoInfoDict["sp"],
                    cNoInfoDict["sp2"],
                    value + cNoInfoDict["tp"],
                    value + cNoInfoDict["tp2"],
                )
            elif type == "QP":
        ...
        usualFrame.pack()
        cfgFrame = {"padx": 3, "pady": 3, "sticky": "nswe"}
        ...
        if True:
            ...
            tk.Label(chargeFrame, text="QP/TP").grid(
                row=3, column=0, sticky="we", **cfg
            )
            ...
            ceraValueE = ttk.Spinbox(
                chargeFrame, from_=0, to=999999, width=int(WIDTH * 7)
            )
            ceraValueE.grid(row=0, column=2, columnspan=1, sticky="we", **cfg)
            ceraValueE.set(0)
            ceraTypeE = ttk.Combobox(
                chargeFrame,
                width=int(WIDTH * 5),
            ...
            ttk.Button(
                chargeFrame, text="充值", command=charge, width=int(WIDTH * 9)
            ).grid(row=2, column=2, sticky="we", **cfg)
            # ttk.Button(chargeFrame,text='清空代币',command=lambda:clear_cera('cera_point')).grid(row=3,column=1,**cfg)
            ttk.Button(
                chargeFrame, text="清空", command=clear_cera, width=int(WIDTH * 9)
            ).grid(row=3, column=2, sticky="we", **cfg)
            # chargeFrame.update()
        ...
        if True:
            cfg = {"padx": 3, "pady": 4}
            PVPgradeE = ttk.Combobox(
                pvpFrame,
                width=int(WIDTH * 7),
                values=[f"{name}" for i, name in enumerate(PVPRankList)],
            )
            PVPgradeE.grid(row=1, column=1, columnspan=1, sticky="we", **cfg)
            readOnlyWidgets.append(PVPgradeE)
            tk.Label(pvpFrame, text="段位").grid(
                row=1, column=2, columnspan=1, sticky="we", **cfg
            )
            ...
            tk.Label(pvpFrame, text="胜场").grid(
                row=2, column=2, columnspan=1, sticky="we", **cfg
            )
            PVPwinPointE = ttk.Spinbox(
                pvpFrame, from_=0, to=999999, width=int(WIDTH * 7)
            )
            PVPwinPointE.grid(row=3, column=1, columnspan=1, sticky="we", **cfg)
            tk.Label(pvpFrame, text="胜点").grid(
                row=3, column=2, columnspan=1, sticky="we", **cfg
            )
            ttk.Button(pvpFrame, text="提交", command=set_PVP).grid(
                row=4, column=1, columnspan=2, sticky="we", **cfg
            )
        ...
        if True:
            ...
            ttk.Button(
                otherFrame,
                text="开启左右槽",
                command=lambda: [sqlM.enable_LR_slot(self.cNo), self.title("开启成功")],
                width=int(WIDTH * 13),
            ).grid(row=2, column=2, **cfg)
            ttk.Button(
                otherFrame,
                text="开启全图全难度",
                command=lambda: [unlock_dungeon(), self.title("开启成功")],
            ).grid(row=2, column=3, **cfg)
            ttk.Button(
                otherFrame,
                text="取消装备等级限制",
                command=lambda: [
    ...
    def buildTab_mail(self, tabView: ttk.Notebook, tabName: str):
        ...
        if True:
            ...
            itemSealVar.set(0)
            itemSealBtn = ttk.Checkbutton(
                itemEditFrame,
                text="封装",
                variable=itemSealVar,
                command=lambda: itemSealVar.get(),
            )
            itemSealBtn.grid(column=1, row=row, **cfg)
            CreateToolTip(itemSealBtn, "无法封装的物品勾选会炸角色")
            itemNameEntry = ttk.Combobox(
                itemEditFrame, width=int(WIDTH * 18), state="normal"
            )
            ...
            tk.Label(itemEditFrame, text=" 耐久：").grid(
                column=3, row=row, padx=cfg["padx"], sticky="w"
            )
            enduranceEntry = ttk.Spinbox(
            ...
            goldLabel.grid(
                column=3, row=row, columnspan=1, padx=cfg["padx"], sticky="w"
            )
        ...
        if True:
            ...
            ttk.Button(
                btnFrame,
                text="发送到当前角色",
                command=lambda: send_mail(self.cNo),
                width=int(WIDTH * 13),
            ).grid(row=2, column=1, **cfg)
            ttk.Button(
                btnFrame,
                text="发送到全服角色",
                command=send_mail_all,
                width=int(WIDTH * 13),
            ).grid(row=2, column=2, **cfg)
            ttk.Button(
                btnFrame,
                text="发送到在线角色",
                command=send_mail_online,
                width=int(WIDTH * 13),
            ).grid(row=2, column=3, **cfg)
            ttk.Button(btnFrame, text="发送到VIP账号", command=send_mail_VIP).grid(
                row=3, column=1, **cfg
            )
            ttk.Button(
                btnFrame, text="发送到VIP角色", command=lambda: send_mail_VIP(all)
            ).grid(row=3, column=2, **cfg)
            ttk.Button(
                btnFrame,
                text="删除全服邮件",
                command=lambda: sqlM.delete_all_mail_cNo(-1),
            ).grid(row=3, column=3, rowspan=1, **cfg)
    ...
    def buildTab_GroupMail(self, tabView: ttk.Notebook, tabName: str):
        ...
        def groupCharge():
            ...
            for i, uid in enumerate(uids):
                ...
                self.title(
                    f"充值{type}-{value} 账号{uid} [{valueNew - value}]->[{valueNew}]]"
                )
            self.update_Info()
        ...
        if True:
            ttk.Button(
                btnFrame,
                text="发送到分组账号",
                command=send_mail_group_a,
                width=int(WIDTH * 13),
            ).grid(row=2, column=1, **cfg)
            ttk.Button(
                btnFrame,
                text="发送到分组角色",
                command=send_mail_group_c,
                width=int(WIDTH * 13),
            ).grid(row=2, column=2, **cfg)
            ttk.Button(
                btnFrame,
                text="发送到分组在线",
                command=send_mail_group_online,
                width=int(WIDTH * 15),
            ).grid(row=2, column=3, rowspan=1, **cfg)
    ...
    def buildTab_event(self, tabView: ttk.Notebook, tabName: str):
        ...
        self.update_event_list_func = lambda: [
            get_available_event(),
            get_running_event(),
        ]
        try:
        ...
        if True:
            ...
            eventNameE = ttk.Combobox(
                eventEditFrame, width=int(WIDTH * 23), state="readonly"
            )
            ...
            ttk.Button(
                eventEditFrame,
                text="刷新服务器活动",
                command=lambda: self.title(f"活动已刷新({len(get_running_event())})"),
            ).grid(row=3, column=2, **cfg)
            ttk.Button(
                eventEditFrame, text="删除选中", width=int(WIDTH * 8), command=del_event
            ).grid(row=3, column=3, **cfg)
            addBtn = ttk.Button(
                eventEditFrame, text="添加活动", width=int(WIDTH * 8), command=set_event
            )
            addBtn.grid(row=3, column=4, **cfg)
            CreateToolTip(addBtn, "添加删除后需重启服务器")

    def buildTab_server(self, tabView: ttk.Notebook, tabName: str):
        self.serverApp = server.ServerCtrlFrame(
            tabView, titlefunc=self.title, autoConnect=self.sshAutoConnect
        )
