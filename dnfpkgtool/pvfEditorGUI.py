...
class WasteframeWidget(ttk.Frame):
    ...
    def _other_build_function(self):
        self.entryTagDict = {
            "[hp recovery]": [
                [self.hpHealTypeE, self.hpE, self.hptimeUseE, self.hpObjE],
                [str, int, int, str],
            ],
            "[mp recovery]": [
                [self.mpHealTypeE, self.mpE, self.mptimeUseE, self.mpObjE],
                [str, int, int, str],
            ],
            "[stat change duration]": [[self.durationE, self.durationObjE], [int, str]],
            "[active status recovery]": [
                [
                    self.activeStatusRecE1,
                    self.activeStatusRecE2,
                    self.activeStatusRecE3,
                ],
                [str, int, str],
            ],
        }
...
class PvfeditstackableframeWidget(ttk.Frame):
    def __init__(self, master=None, **kw):
        ...
        self.editedTree.bind(
            "<<TreeviewSelect>>", self.select_treeview_item_Event, add=""
        )
        ...
        self.stkTypeE.bind("<<ComboboxSelected>>", self.select_stkType, add="+")
        ...
        label35.configure(anchor="center", justify="left", text="最低等级", width=8)
    ...
    def _build_other_functions(self):
        ...
        CreateToolTip(
            self.searchNameE, textFunc=lambda: self.getItemPVFInfo("输入物品名进行搜索")
        )
        CreateToolTip(
            self.idE, textFunc=lambda: self.getItemPVFInfo("输入物品ID进行搜索")
        )
        ...
        self.exFramesDict = {"[waste]": WasteframeWidget}
        ...
        self.rarityE.config(
            values=[f"{key}-{value}" for key, value in rarityMap.items()]
        )
    ...
    def save_file_edit(self):
        ...
        if (
            self.currentLeaf.get("jsonEditorServer") is not None
            and self.currentLeaf["jsonEditorServer"].running
        ):
    ...
    def open_advance_edit(self):
        ...
        def start_json_thread():
            ...
            self.currentLeaf["jsonEditorServer"] = jsoneditor.editjson(
                self.currentLeaf["itemInDict"],
                save,
                options={"mode": "code"},
                title=self.currentLeaf["itemInDict"].get("[name]"),
                run_in_thread=True,
            )
            localLeaf = self.currentLeaf

        if (
            self.currentLeaf.get("jsonEditorServer") is None
            or self.currentLeaf["jsonEditorServer"].running == False
        ):
            self.save_file_edit()
            start_json_thread()
        else:
    ...
    def searchItem(self, e: tk.Event):
        ...
        if len(key) > 0:
            ...
            self.searchNameE.config(
                values=[str([item[0]]) + " " + item[1] for item in res]
            )
...
class PvfeditequipmentframeWidget(ttk.Frame):
    def __init__(self, master=None, **kw):
        ...
        label23.configure(anchor="center", justify="left", text="最低等级", width=8)
        ...
        label25.configure(anchor="center", justify="left", text="要求技能", width=8)
        ...
        label28.configure(anchor="center", justify="left", text="魔法封印", width=8)
        ...
        label42.configure(anchor="center", justify="left", text="物理攻击", width=8)
        ...
        label43.configure(anchor="center", justify="left", text="魔法攻击", width=8)
        ...
        label44.configure(anchor="center", justify="left", text="独立攻击", width=8)
        ...
        label45.configure(anchor="center", justify="left", text="攻击速度", width=8)
        ...
        label47.configure(anchor="center", justify="left", text="吟唱速度", width=8)
        ...
        label48.configure(anchor="center", justify="left", text="移动速度", width=8)
        ...
        label31.configure(anchor="center", justify="left", text="物理防御", width=8)
        label31.pack(side="left")
        ...
        label33.configure(anchor="center", justify="left", text="魔法防御", width=8)
        label33.pack(side="left")
        ...
        self.skillJobE1.bind("<<ComboboxSelected>>", self.select_skill_job, add="")
        ...
        self.skillJobE2.bind("<<ComboboxSelected>>", self.select_skill_job, add="")
        ...
        self.skillJobE3.bind("<<ComboboxSelected>>", self.select_skill_job, add="")
        ...
        self.skillJobE4.bind("<<ComboboxSelected>>", self.select_skill_job, add="")
    ...
    def _build_other_functions(self):
        ...
        self.entryTagDict = {
            ...
            "[equipment physical defense]": [
                [self.phyDefenseE1, self.phyDefenseE2],
                int,
            ],
            "[equipment magical defense]": [
                [self.magicDefenseE1, self.magicDefenseE2],
                int,
            ],
    ...
    def fillFrameWithItemLeaf(self, itemLeaf: dict):
        ...
        if itemInDict.get(tag) is not None:
            for i, value in enumerate(itemInDict.get(tag)):
                try:
                    if i % 3 == 0:
                        itemInDict.get(tag)[i] = (
                            f"{self.jobTagDictRev.get(value)}-{value}"
                        )
                        jobID = self.jobTagDictRev.get(value)
                    elif i % 3 == 1:
                        itemInDict.get(tag)[i] = (
                            f"{value}-{cacheM.PVFcacheDict.get('skill').get(jobID).get(value).get('[name]')}"
                        )
                except:
                    pass
    ...
    def update_with_PVF(self):
        ...
        self.jobTagDictRev = {
            value: key for key, value in cacheM.PVFcacheDict.get("jobTagDict").items()
        }
        for skillEntrys in self.skillEntrys:
            skillEntrys[0][0].config(
                values=[
                    f"{key}-{value}"
                    for key, value in cacheM.PVFcacheDict.get("jobTagDict").items()
                ]
            )
    ...
    def open_advance_edit(self):
        ...
        if (
            self.currentLeaf["jsonEditorServer"] is None
            or self.currentLeaf["jsonEditorServer"].running == False
        ):
            self.read_and_save_file_edit()
            start_json_thread()
...
class PvfeditmainframeApp:
    def __init__(self, master=None):
        ...
        self.fileMenu.add("command", command=self.resave_PVF_edit, label="另存编辑")
    ...
    def open_PVF(self):
        def inner():
            ...
            if PVF != "" and p.exists():
                ...
                pvf: TinyPVFEditor = cacheM.loadItems2(
                    True, PVF, retType="pvf", encode=self.encodeE.get()
                )
                self.pvf = pvf
                self.PVFE.delete(0, tk.END)
                self.PVFE.insert(0, p)
                stkLstPath = "stackable/stackable.lst"
                self.lstDict["stackable"] = LstEditor(
                    pvf.read_File_In_Decrypted_Bin(stkLstPath),
                    pvf.stringTable,
                    baseDir="stackable",
                    suffix=".stk",
                )
                ...
                self.stkTab.searchNameE.insert(
                    0, f"输入关键词检索道具({len(cacheM.stackableDict.items())})"
                )
                self.stkTab.enable_Btns()

                equLstPath = "equipment/equipment.lst"
                self.lstDict["equipment"] = LstEditor(
                    pvf.read_File_In_Decrypted_Bin(equLstPath),
                    pvf.stringTable,
                    baseDir="equipment",
                    suffix=".equ",
                )
                ...
                self.equTab.searchNameE.insert(
                    0, f"输入关键词检索装备({len(cacheM.equipmentDict.items())})"
                )
    ...
    def open_PVF_edit(self):
        ...
        path = askopenfilename(
            filetypes=[("DNF PVF编辑缓存.pvfedit file", "*.pvfedit")]
        )
