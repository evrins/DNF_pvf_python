...
class SkilleditframeWidget(ttk.Frame):
    def __init__(self, master=None, **kw):
        ...
        self.jobTreeV.configure(height=11, selectmode="browse", show="headings")
        self.jobTreeV_cols = ["column4", "column5"]
        self.jobTreeV_dcols = ["column4", "column5"]
        self.jobTreeV.configure(
            columns=self.jobTreeV_cols, displaycolumns=self.jobTreeV_dcols
        )
        self.jobTreeV.column(
            "column4", anchor="center", stretch="true", width=40, minwidth=20
        )
        self.jobTreeV.column(
        ...
        self.skillTreeV.configure(
            columns=self.skillTreeV_cols, displaycolumns=self.skillTreeV_dcols
        )
        self.skillTreeV.column(
            "column6", anchor="center", stretch="true", width=40, minwidth=20
        )
        self.skillTreeV.column(
            "column7", anchor="center", stretch="true", width=120, minwidth=20
        )
        self.skillTreeV.column(
            "column8", anchor="center", stretch="true", width=60, minwidth=20
        )
        ...
        self.growTypeTreeV.configure(
            columns=self.growTypeTreeV_cols, displaycolumns=self.growTypeTreeV_dcols
        )
        self.growTypeTreeV.column(
            "column9", anchor="center", stretch="true", width=40, minwidth=20
        )
        self.growTypeTreeV.column(
            "column10", anchor="center", stretch="true", width=80, minwidth=20
        )
        self.growTypeTreeV.column(
            "column14", anchor="w", stretch="true", width=200, minwidth=20
        )
        ...
        self.growTypeTreeV.bind("<<TreeviewSelect>>", self.select_growType, add="")
        self.growTypeSkillTreeV = ttk.Treeview(frame3)
        self.growTypeSkillTreeV.configure(selectmode="extended", show="headings")
        ...
        self.growTypeSkillTreeV.column(
            "column11", anchor="center", stretch="true", width=40, minwidth=20
        )
        self.growTypeSkillTreeV.column(
            "column12", anchor="center", stretch="true", width=120, minwidth=20
        )
        self.growTypeSkillTreeV.column(
            "column13", anchor="center", stretch="true", width=60, minwidth=20
        )
        ...
        self.growTypeSkillTreeV.bind(
            "<<TreeviewSelect>>", self.select_skill_growType, add=""
        )
    ...
    def load_pos(self):
        ...
        for jobID, jobTag in cacheM.PVFcacheDict.get("jobTagDict").items():
            ...
            for characJobSegDict in skillSpLeaf.get("itemInList"):
                if not isinstance(characJobSegDict, dict):
                    continue
                skillInfoSegs = characJobSegDict.get("[character job]")
                if skillInfoSegs is None:
                    continue
                skillInfoSegs = skillInfoSegs[2:]
                for skillInfoSeg in skillInfoSegs:
                    if not isinstance(skillInfoSeg, dict):
                        continue
                    # print(skillInfoSeg)
                    skillID = self.pvf.get_seg(
                        skillInfoSeg.get("[skill info]"), "[index]"
                    )[0]
                    skillPos = self.pvf.get_seg(
                        skillInfoSeg.get("[skill info]"), "[icon pos]"
                    )
    ...
    def fillFrameWithItemLeaf(self, itemLeaf: dict):
        ...
        for tag, entryAndTypes in self.entryTagDict.items():
            if (
                tag == "[skill fitness second growtype]"
                and itemInDict.get(tag) is not None
            ):
    ...
    def select_growType(self, e):
        def get_Skill_List(itemInList: list) -> list:
            ...
            for characJobSegDict in itemInList:
                if characJobSegDict.get("[character job]")[1] == growTypeName:
                    for skillSeg in characJobSegDict.get("[character job]")[2:]:
                        if not isinstance(skillSeg, dict):
                            continue
            ...
            skillList = [
                [skillID, item[0], item[1]] for skillID, item in skillDict.items()
            ]
            skillList.sort()
            return skillList
        ...
        growTypeNo, growTypeName, spPath = self.growTypeTreeV.item(selectedItem)[
            "values"
        ]
        selectedItem = self.jobTreeV.selection()
        jobID = self.jobTreeV.item(selectedItem)["values"][0]
    ...
    def save_growtype_skill(self):
        ...
        if spPath is not None:
            ...
            for child in self.growTypeSkillTreeV.get_children():
                skillID, skillName, skillPath = self.growTypeSkillTreeV.item(child)[
                    "values"
                ]
                ...
                if growtype_maximum_level_2 is not None:
                    growtype_maximum_level_2 = [max(growtype_maximum_level_2)] * len(
                        growtype_maximum_level_2
                    )
                ...
                print(skillInfoSeg)
            character_job_seg = {
                "[character job]": character_job_seg_old.get("[character job]")[:2]
                + character_job_seg_tmp
                + [True]
            }
