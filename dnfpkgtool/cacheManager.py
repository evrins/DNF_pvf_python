...
class PVFCacheManager:
    ...
    def __init__(self) -> None:
        self.tinyCache = {
            "_cacheVersion": PVF_CACHE_VERSION
        }  # MD5: nickName/pvfPath/equNum/stkNum/fileName
        self.loadCacheList()
...
def equipmentDetailDict_transform(
    equipmentStructuredDict_: dict = None, globalChange=True
):
    if globalChange:
        ...
        if dir1Name == "common":  # 处理防具和首饰
            for commonType, partDir in dir2Dict.items():
                ...
                if commonType in ["amulet", "wrist", "ring"]:  # 首饰
                    ...
                    add_dict_all(
                        equipmentForamted["特殊装备"][keyMap[commonType]], partDir
                    )
                else:  # 是防具
                    for armorType, itemDict in partDir.items():
                        if not isinstance(
                            ...
                            continue
                        add_dict_all(
                            equipmentForamted["防具"][keyMap[armorType]][
                                keyMap[commonType]
                            ],
                            partDir,
                        )
            ...
            if not isinstance(jobDirsDict, dict):
                continue
            for weaponDirName, weaponTypeDict in jobDirsDict.items():
                ...
                if weaponDirName == "weapon":
                    for weaponType, weaponDict in weaponTypeDict.items():
                        ...
                        targetDict = equipmentForamted["武器"][keyMap[jobName]][
                            keyMap[weaponType]
                        ]
        ...
        if isinstance(enhanceSeg, dict):
            ...
            for enhanceKey, enhanceValueInList in enhanceSeg.items():
                ...
                if isinstance(enhanceValueInList, list):
                    enhanceValueInList = [
                        string_2_Zh(string) for string in enhanceValueInList
                    ]
    ...
    if usePVF:
        ...
        if MD5 in cacheManager.allMD5():
            ...
            if useCache and MD5 in cacheManager.allMD5():
                ...
                PVFcacheDict["stackable_detail"] = all_items_dict.pop(
                    "stackable_detail"
                )
                PVFcacheDict["equipment_detail"] = all_items_dict.pop(
                    "equipment_detail"
                )
                PVFcacheDict["nickName"] = "None"
                PVFcacheDict["MD5"] = MD5
                PVFcacheDict["equipmentStructuredDict"] = all_items_dict.pop(
                    "equipmentStructuredDict"
                )
                ...
                PVFcacheDict["dungeonName"] = dungeonDict_Convert(
                    PVFcacheDict["dungeon"]
                )
        ...
        csvList = list(
            filter(
                lambda item: item.name[-4:].lower() == ".csv",
                [item for item in csvPath.iterdir()],
            )
        )
